"""导出到课件：一键生成课件包（含向导）。
包内容：
  预览.png      当前画布渲染图 + "▶ 点击运行" 角标
  草图.json     草图数据（启动器靠它还原现场）
  运行.bat      双击/在 PPT 里点预览图 → 拉起完整版应用（功能 100%）
  交互.html     浏览器交互版（JSXGraph，可拖动，支持几乎所有对象，离线可用）
  jsxgraphcore.js / jsxgraph.css  交互网页的几何引擎（离线）
  使用说明.txt
"""
import json
import math
...
def _py_expr_to_js(expr, var_names):
    """把 Python 数学表达式转换为 JSXGraph 可用的 JS 表达式。"""
    s = expr.replace("^", "**")
...
def _jsx_export(doc, include_vars=True):
    """把文档翻译成 JSXGraph 代码（支持点/线/圆/多边形/贝塞尔/椭圆/函数等）。"""
    lines = []
    names = {}
    counter = [0]
    ...
    # 导出变量为 slider（如果 include_vars 为 False，则创建不可见的 slider 供表达式引用）
    ...
    # 计算视窗范围
    xs = [o.x for o in doc.objects if type(o).__name__ in ("FreePoint", "PointOnObject", "Midpoint", "ExprPoint") and o.exists]
    ys = [o.y for o in doc.objects if type(o).__name__ in ("FreePoint", "PointOnObject", "Midpoint", "ExprPoint") and o.exists]
    if xs and ys:
        m = max(max(xs) - min(xs), max(ys) - min(ys), 4.0) * 0.7 + 2.0
        cx, cy = (min(xs) + max(xs)) / 2, (min(ys) + max(ys)) / 2
        bb = [cx - m, cy + m, cx + m, cy - m]
    else:
        bb = [-10, 10, 10, -10]
    ...
    var_names = all_var_names  # 用于表达式替换
    ...
    for obj in doc.objects:
        ...
        if tn in ("FreePoint", "ExprPoint"):
            ...
            if host_name:
                n = nv(); names[obj.id] = n
                lines.append(f"var {n} = board.create('glider', [{obj.x:.4f}, {obj.y:.4f}, {host_name}], "
                             f"{{name: 'P{obj.id}', size: 2}});")
            ...
            if a and b:
                ...
                lines.append(f"var {n} = board.create('line', [{a}, {b}], "
                             f"{{strokeColor: '#495057', strokeWidth: 2, straightFirst: true, straightLast: true}});")
            ...
            if origin and through:
                ...
                lines.append(f"var {n} = board.create('line', [{origin}, {through}], "
                             f"{{strokeColor: '#0b7285', strokeWidth: 2, straightFirst: false, straightLast: true}});")
            ...
            if c and v:
                ...
                lines.append(f"var {n} = board.create('regularpolygon', [{c}, {v}, {obj.n}], "
                             f"{{fillColor: '#ff6b6b', fillOpacity: 0.25, strokeColor: '#ff6b6b', strokeWidth: 2}});")
            ...
            if c_name and a_name and b_name:
                ...
                code = (
                    ...
                    f"function(t) {{ return {c_name}.X() + ({a_name}.X() - {c_name}.X()) * Math.cos(t) + ({b_name}.X() - {c_name}.X()) * Math.sin(t); }}, "
                    f"function(t) {{ return {c_name}.Y() + ({a_name}.Y() - {c_name}.Y()) * Math.cos(t) + ({b_name}.Y() - {c_name}.Y()) * Math.sin(t); }}, "
            ...
            if p0 and p1 and p2 and p3:
                ...
                code = (
                    f"var {n} = board.create('curve', ["
                    f"function(t) {{ var u=1-t; return u*u*u*{p0}.X() + 3*u*u*t*{p1}.X() + 3*u*t*t*{p2}.X() + t*t*t*{p3}.X(); }}, "
                    f"function(t) {{ var u=1-t; return u*u*u*{p0}.Y() + 3*u*u*t*{p1}.Y() + 3*u*t*t*{p2}.Y() + t*t*t*{p3}.Y(); }}, "
            ...
            if obj.kind == "explicit":
                lines.append(f"var {n} = board.create('functiongraph', [function(x) {{ return {js_expr}; }}], "
...
def _fetch_jsxgraph(dest_dir: Path) -> bool:
    """下载 JSXGraph 到包内（离线可用）；失败返回 False（回退 CDN）。"""
...
def _make_launcher(dest_dir: Path, json_name: str):
    ...
    (dest_dir / "运行.bat").write_text(bat, encoding="gbk", errors="ignore")


def _do_export(canvas, folder, gen_preview=True, gen_web=True, gen_launcher=True, gen_readme=True,
               preview_w=1280, preview_h=800, add_badge=True, offline=True, include_vars=True, parent=None):
    doc = canvas.doc
    ...
    if gen_web:
        ...
        if not offline_ok:
            ...
            html = html.replace('<link rel="stylesheet" href="jsxgraph.css">', f'<link rel="stylesheet" href="{JSX_CSS_URL}">')
    ...
    if gen_readme:
        readme = (
            ...
            "3. 放映时点击预览图，即自动打开 GeoSketch 并载入本草图（可拖动、变量、表达式全功能）。\n\n"
            "【浏览器版（无需安装）】\n"
            "双击\"交互.html\"即可在浏览器中打开，点可拖动（支持多边形、贝塞尔、椭圆、函数等）。\n"
            + ("" if offline else "注意：本机未能下载 JSXGraph，交互.html 需联网才能打开。\n")
