...
class Sup:
    def __init__(self, base, script):
        self.base, self.script = base, script
...
SYMBOLS = {
    ...
    "cdot": "·", "times": "×", "div": "÷", "pm": "±", "infty": "∞",
    "partial": "∂", "le": "≤", "ge": "≥", "ne": "≠", "approx": "≈",
    "equiv": "≡", "sim": "∼", "rightarrow": "→", "leftarrow": "←",
    "in": "∈", "subset": "⊂", "cup": "∪", "cap": "∩",
...
def tokenize(s):
    ...
    while i < n:
        ...
        if c.isspace():
            ...
            toks.append(("SUP", "^"))
            toks.append(("NUM", c.translate(_SUP)))
            i += 1

        elif c in "₀₁₂₃₄₅₆₇₈₉":
            toks.append(("SUB", "_"))
            toks.append(("NUM", c.translate(_SUB)))
            i += 1
...
class _Parser:
    ...
    def parse(self):
        return self.row(False, False)

    def row(self, in_group, paren=False):
        nodes = []
        stop = "RP" if paren else "RB"

        while True:
            ty, v = self.peek()

            if ty == "EOF" or (in_group and ty == stop):
                break
        ...
        if in_group and self.peek()[0] == stop:
            self.take()
    ...
    def group(self):
        self.take()  # {
        return self.row(True, False)

    def paren(self):
        self.take()  # (
        inner = self.row(True, True)
        return Paren(inner)
    ...
    def _cmd(self, name):
        ...
        if name in GREEK:
            return Ord(GREEK[name], "var")

        if name in SYMBOLS:
            return Ord(SYMBOLS[name], "sym")
